...
мэр.post('/sleiv/poimat', function(req, res) {
    ...
    гонецОтправлятель.post('/slaveql', пойматьОбернуть(протеретьОтПыли(слейв.неПокажу()))).then(() => {
        res.json({ok: true});
    }).catch((err) => {
...
мэр.post('/sleiv/torganut', function(req, res) {
    ...
    гонецОтправлятель.post('/slaveql', передатьОбернуть(протеретьОтПыли(трейдСлейва.неПокажу()))).then(() => {
        res.json({ok: true});
    }).catch((err) => {
        if (err.response.status === 500) {
            res.status(500).end();
...
мэр.post('/torg/prinyat', function(req, res) {
    ...
    гонецОтправлятель.post('/slaveql', принятьОбернуть(протеретьОтПыли(трейдСлейва.неПокажу()))).then(() => {
        res.json({ok: true});
    }).catch((err) => {
        if (err.response.status === 500) {
            res.status(500).end();
...
мэр.post('/babosi/otpravit', function(req, res) {
    ...
    гонецОтправлятель.post('/slaveql', заплатитьЧеканнойМонетойОбернуть(протеретьОтПыли(отправкаБабосов.неПокажу()))).then(() => {
        res.json({ok: true});
    }).catch((err) => {
        if (err.response.status === 500) {
            res.status(500).end();
